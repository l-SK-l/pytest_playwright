...
class JournalsPage:
    ...
    def open_system_section(self) -> None:
        ...
        self.page.get_by_role("button", name="System", exact=True).click(timeout=60000)
        self.page.locator("#btnStatusRefresh").get_by_role("button")
        self.page.wait_for_timeout(5000)

    def open_ids_section(self) -> None:
        self.page.get_by_role("button", name="Journals").click(timeout=60000)
        self.page.get_by_role("button", name="IDS", exact=True).click(timeout=60000)
        self.page.locator("#btnStatusRefresh").get_by_role("button")
        self.page.wait_for_timeout(5000)
    
    def open_management_section(self) -> None:
        self.page.get_by_role("button", name="Journals").click(timeout=60000)
        self.page.get_by_role("button", name="Management", exact=True).click(timeout=60000)
        self.page.locator("#btnStatusRefresh").get_by_role("button")
        self.page.wait_for_timeout(5000)

    def select_all_visible_rows_system(self) -> None:
        self.page.get_by_role("row", name=
                              "Date Security node Device ID Facility Messages Category Severity Severity weight").get_by_role("button").click()
    
    def select_all_visible_rows_ids(self) -> None:
        self.page.get_by_role("row", name=
                              "Date Action Security node Source address Source country Destination address Destination country Destination domain Protocol Destination port Signature").get_by_role("button").click()

    def select_all_visible_rows_management(self) -> None:
        self.page.get_by_role("row", name=
                              "Date Security node Device ID Subject Message Category Severity Severity weight").get_by_role("button").click()

    def open_filters(self) -> None:
        self.page.locator("#btnFilter").get_by_role("button").click(timeout=60000)
    ...
    def clear_all(self) -> None:
        ...
        self.page.get_by_role("button", name="Clear all").get_by_role("button", name="Clear all", exact=True).filter(has_text="Clear all").click()
        self.page.get_by_role("button", name="Proceed", exact=True).click()
    ...
    def wait_and_confirm(self, path: str) -> None:
        with self.page.expect_download() as download_info:
            ...
            self.page.get_by_role("button", name="Export CSV logs").first.click(timeout=300000)
